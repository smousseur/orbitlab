...
/**
 * Orbit camera tuning parameters controlling world-unit scaling, input sensitivity,
 * distance clamping, and dynamic frustum adjustment. Defaults are intentionally
 * conservative and meant to be adjusted per use case.
 *
...
public record OrbitCameraConfig(
  ...
  /**
   * Computes the default camera distance from the target based on
   * the system radius and the default distance factor.
   *
  ...
  /**
   * Creates a default orbit camera configuration for the given system radius,
   * with conservative defaults suitable for solar system visualization.
   *
   * @param systemRadiusWorldUnits the solar system radius in JME world units
