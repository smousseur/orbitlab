 ...
 *
 * <p>The three reasons are independent and can occur together, which is the whole reason the
 * texture is checked at all: a re-export that keeps the mesh and swaps the texture leaves the frame
 ...
 * @param mirrored whether the texture now runs the other way round the pole — the chirality's sign
 *     changed — or could not be measured at all. A mirrored map keeps its pole and its prime
 *     meridian, so the frame angle alone reads zero on it
 */
public record MeshDivergence(
    SolarSystemBody body, float frameDeviationDeg, boolean textureChanged, boolean mirrored) {}
