...
public final class OrbitLineFactory {
  ...
  /**
   * Builds a heliocentric line-strip geometry from an orbit path. The orbit positions are rendered
  ...
  /**
   * Builds a closed line-loop geometry from a list of position vectors, rendered relative to a
   * given solar system body.
   *
  ...
  /**
   * Builds a line-strip geometry from an orbit path, optionally centered on a given body. When a
   * center body is provided, each position is adjusted by subtracting the center body's
   * heliocentric position at the corresponding time step.
   *
