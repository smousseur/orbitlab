 ...
 * the stack's dry mass, the parking insertion planned two burns it could not fly, and the phase went
 * on to propagate 2 666 s of pure ballistics from 36 km at 7 603 m/s. The evaluation hung for four
 * hours instead of reporting an infeasible λ in seconds. {@code
 ...
 *
 * <p><b>It only has to stop.</b> The verdict machinery downstream is already in place: an early STOP
 * shows up as {@code StageChainRunner.StageRun.shortfallSeconds() > 0}, which makes {@code
...
public final class ReentryGuard {
   ...
   * and referenced to the equatorial radius, while the Earth is 21.4 km oblate ({@code Re − Rp =
   * 6 378 137 − 6 356 752}). The spherical altitude of a launch site is therefore structurally
   * negative before liftoff — −0.16 km at Kourou, −4.9 km at Canaveral, −11.1 km at Baikonur,
   * −17.0 km at Plesetsk. The trap is not the ascent climbing away from a naive floor; it is that
   * the pad already sits underneath it. At −50 km every terrestrial launch site clears the floor by
   * at least 33 km, so ascent phases need no exclusion, no radial-velocity test and no activation
   * date — {@code g} is positive and increasing from {@code t = 0}.
   *
   ...
   *
   * <p><b>Why WARN and not ERROR</b>, unlike {@link DepletionGuard#arm}: that guard firing means the
   * upstream mass accounting is wrong, i.e. a bug. A re-entry does not — it is a physically
   ...
   * truncation itself penalizes them through the cost function, and a line per candidate would flood
   * the output. Mirrors {@link DepletionGuard#armQuiet}.
   *
   * @param propagator the propagator to guard
   */
