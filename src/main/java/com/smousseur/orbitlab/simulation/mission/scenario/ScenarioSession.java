 ...
 * single current mission is destroyed, so a corrupt file never costs the session on screen.
 */
public final class ScenarioSession {
   ...
   * has one implementation. The caller — the scenario {@code AppState} — supplies {@code
