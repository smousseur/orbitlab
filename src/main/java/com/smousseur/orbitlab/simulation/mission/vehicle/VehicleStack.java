 ...
 * code stating something false about the model, which is exactly the mistake recorded below.
 *
 ...
 *     {@code LauncherModel.instantiate}
 */
public record VehicleStack(List<Vehicle> vehicles, StagingPlan stagingPlan) implements Vehicle {
   ...
   * is supported somewhere. It is not, anywhere.
   *
   ...
   * nothing, it only stops being written by hand.
   */
   ...
   * stack-wide total. Given the current mass, the active stage holds {@link
   ...
   * total consumed, so this stays a pure function of the current mass.
   *
