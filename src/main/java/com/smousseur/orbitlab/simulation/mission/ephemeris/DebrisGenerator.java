...
/**
 * Turns {@link JettisonEvent}s into {@link DebrisTrack}s by propagating each jettisoned object on
 * its own after separation (PHY-5 / L1, spec {@code docs/multi-objets/03-conception-L1.md} §2.3).
 * Runs only on the replay pass, never in the optimizer loop.
 *
...
public final class DebrisGenerator {
   ...
   * @return one track per event
   */
  public List<DebrisTrack> generate(
      List<JettisonEvent> events, AtmosphereModel atmosphere, double horizonSeconds) {
    List<DebrisTrack> tracks = new ArrayList<>(events.size());
    for (JettisonEvent event : events) {
      tracks.add(new DebrisTrack(propagate(event, atmosphere, horizonSeconds)));
    }
    return List.copyOf(tracks);
  }

  private MissionEphemeris propagate(
      JettisonEvent event, AtmosphereModel atmosphere, double horizonSeconds) {
    OrekitService service = OrekitService.get();
    DragContext drag =
        atmosphere == AtmosphereModel.NONE ? null : new DragContext(event.aero(), atmosphere);
    ...
    propagator.setInitialState(event.initialState());
    ...
    AbsoluteDate start = event.initialState().getDate();
    ...
    if (points.size() < 2) {
      points.add(0, sampleOf(event.initialState(), ellipsoid));
    }
