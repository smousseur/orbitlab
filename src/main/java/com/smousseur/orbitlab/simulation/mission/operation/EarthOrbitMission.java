 ...
 *
 * <p>This class was {@code LEOMission} before MIS-7. The rename is a rename: the three factories
 * and their stage chains are unchanged, they now carry the target plane instead of recomputing
 * {@code toRadians(latitude)} at each call site. {@link LaunchPlane#dueEast} reproduces the
...
public class EarthOrbitMission extends EarthMission {
   ...
   * on when the plane residual is cleaned up.
   *
   * <p><b>Payload delivery.</b> When the payload carries usable propellant, the upper stage is
   * dropped after the transfer — and after the plane trim, which stays on it — and the payload
   * flies the final trim on its own engine: the mission actually delivers its satellite. An inert
   * payload keeps the upper stage to the end, so the chain is byte-identical to before and the
   * zero-tolerance gates are untouched.
   *
