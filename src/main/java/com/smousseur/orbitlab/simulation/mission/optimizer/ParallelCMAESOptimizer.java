 ...
 *
 */
public class ParallelCMAESOptimizer
   ...
   /**
     ...
     * wrapper, so execution order does not change the outcome. When {@code parallelGeneration} is
     * off (or the batch is trivial) the batch runs sequentially, still off the counter — identical
     * result, no pool.
     *
     ...
     */
    private boolean evaluateGeneration(final FitnessFunction fitfun,
    ...
    private class FitnessFunction {
         ...
         * single-threaded by {@link ParallelCMAESOptimizer#evaluateGeneration}, so this may run on
         * the pool without racing the non-thread-safe {@code Incrementor}. {@code rawObjective} is
         * the same objective the base was given, so the value is identical to {@code value}.
         *
         * @param point Normalized objective variables.
         * @return the objective value + penalty for violated bounds.
         */
