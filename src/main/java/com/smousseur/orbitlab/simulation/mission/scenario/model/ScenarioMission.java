...
/**
 * One saved mission, sealed on <b>the same hierarchy as {@code MissionSpec}</b> — same two
 ...
 * exhaustively with no default branch, so the day a lunar {@code MissionSpec} branch appears, the
 * compilation fails until the matching record exists here. A new mission type cannot silently
 * become unpersistable.
 *
...
public sealed interface ScenarioMission permits ScenarioMission.EarthOrbit, ScenarioMission.Geo {
