 ...
 *
 * <p><b>A solver holds no state of its own</b>: the grid, the evaluation cache and the criteria all
 * belong to {@link Run}, one per call. An instance is therefore reusable and reentrant, and two
 * threads may search the same solver at once — which matters because these searches are started
 * from wherever a mission is scheduled: the render thread through the wizard, the init thread for
 * the lunar demonstration, and the optimizer thread the day a window is recomputed at launch.
 *
 * <p><b>What that does not make thread-safe is the problem.</b> A closed-form criterion is usually
 * immutable and shareable; one whose {@link LaunchWindowProblem#confirm} flies a mission stage is
 * not, and two searches sharing it would fly the same mission twice at once. The rule belongs to
 * the problem, and the solver does not pretend to fix it.
 */
public class LaunchWindowSolver {
  ...
  public List<LaunchWindow> solve(LaunchWindowSearch search) {
    return new Run(problem, search).solve();
  }

  /**
   * One execution of one search — the grid, the memoised evaluations, and the criteria they are
   * read against.
   *
   * <p>It exists so that {@link LaunchWindowSolver} can carry nothing but its problem. Holding the
   * search on the solver worked as long as one thread ran one search at a time, and would have
   * failed silently rather than loudly the day two overlapped: the second call would have reset the
   * cache under the first, which does not throw, it merely returns a window measured against two
   * different searches.
   */
  private static final class Run {
    private final LaunchWindowProblem problem;
    private final LaunchWindowSearch search;
    private final Map<Long, LaunchWindowCandidate> evaluations = new HashMap<>();

    private Run(LaunchWindowProblem problem, LaunchWindowSearch search) {
      this.problem = problem;
      this.search = search;
    }

    private List<LaunchWindow> solve() {
