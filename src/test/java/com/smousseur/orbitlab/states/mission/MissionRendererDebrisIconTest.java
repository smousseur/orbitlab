 ...
 * globe, not at the frozen inertial pose of the impact instant, which the rotating Earth drifts out
 * from under. Tests the pure frame algebra of {@link MissionRenderer#rotateWithGlobe} on both a
 * position and a direction; the on-screen placement itself is judged by eye.
 */
class MissionRendererDebrisIconTest {
  ...
  void landedPoseRidesTheTurningGlobeToTheSameGroundPoint() {
    ...
    // ...and it stays on the SAME body-fixed ground point throughout: de-rotating each by its own
    // drawn rotation yields the one seam-local coordinate.
    ...
    assertTrue(
        ...
        () -> "the icon must stay on its one body-fixed ground point, seam " + seamAtNow);
  }
}
