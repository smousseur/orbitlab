...
class ScenarioCodecTest {
  ...
  private static ScenarioMission.EarthOrbit leo(
    ...
    return leo(horizonDays, inclinationDeg, raanDeg, solution, null);
  }
  ...
  void absentValues_areOmittedFromTheText() {
    ...
    assertFalse(json.contains("deorbit"), json);
  ...
  void absentValues_comeBackAbsent() {
    ...
    assertNull(read.deorbit());
  }

  @Test
  void deorbitFlag_writesAPropertyAndReadsBackTrue() {
    String json = ScenarioCodec.write(fileOf(leo(null, null, null, null, Boolean.TRUE)));
    assertTrue(json.contains("\"deorbit\""), json);

    ScenarioMission.EarthOrbit read =
        (ScenarioMission.EarthOrbit) ScenarioCodec.read(json).missions().getFirst();
    assertEquals(Boolean.TRUE, read.deorbit());
  }

  /**
   * A mission saved before this field existed carries no {@code deorbit} property at all. The text
   * below is the shape {@link #absentValues_areOmittedFromTheText} already proves the writer
   * produces, copied literally rather than generated by that same writer at test time, so the
   * fixture does not depend on the very code it is meant to guard against.
   */
  @Test
  void fileWithoutTheDeorbitProperty_readsAsNotDeorbited() {
    String json =
        """
        {
          "formatVersion" : 1,
          "savedAt" : "2026-08-21T14:32:10Z",
          "clockDate" : "2030-03-01T05:30:00Z",
          "missions" : [ {
            "type" : "LEO",
            "name" : "LEO 400",
            "launchDate" : "2030-03-01T12:00:00Z",
            "site" : {
