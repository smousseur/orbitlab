...
**L'UI ne compte alors plus qu'en opportunités.** Elle demande *trois créneaux*, pas *trois jours* ; le planificateur dérive l'horizon de la récurrence du problème, et l'axe se tend du plancher à la fermeture du dernier créneau rendu, plus un douzième de **cette portée-là** — pas un douzième de récurrence, ce qui ferait environ le quart d'une récurrence pour trois opportunités.

**Ce que le widget garde tout de même, et il faut être exact.** Une seule constante d'affichage vit chez lui : le nombre de marqueurs à montrer. La règle est que le widget ne décide d'aucun **horizon**.

**La demi-largeur du zoom était la deuxième, et elle était fausse.** Elle valait 300 s, fixées ici à ± 5 min et assumées comme une durée terrestre. Elles ne l'étaient pas : le créneau vaut `2·marge/pente` avec `pente = v·sin(i)·ω⊕`, donc il croît en `1/sin i` — 3 min 48 s à 51,6°, 6 min 15 s à 28,5°, et **32 min 40 s** pour un tir plein est depuis Kourou, qui est le site par défaut du wizard. Le cadre débordait alors, la fraction était bornée à [0, 1] et le bord gauche de la légende à la piste, si bien que `opens` se posait exactement sur l'heure de début de cadre. Le cas avait été prévu et écarté comme « inatteignable sur le critère terrestre », en raisonnant sur les 232 s mesurés à 51,6° comme s'ils étaient une propriété du critère et non d'une inclinaison. La demi-largeur est donc devenue **adaptative et quantifiée** (`ZoomScale`) : le plus petit barreau d'une échelle de subdivisions lisibles de la seconde, de la minute et de l'heure — de ± 10 s à ± 6 h — qui contient le créneau sans collision de légendes, et la note du bandeau le nomme. Quantifiée et non continue : un cadre valant 1,3 fois le créneau dessinerait la même image quelle que soit l'inclinaison, et la largeur du créneau *relative au cadre* cesserait de signifier quoi que ce soit. Au niveau du jour, une opportunité est dessinée comme un **instant** — un marqueur, dont personne n'attend que la largeur signifie quelque chose, donc qui ne ment sur rien. La largeur vraie ne se montre qu'au niveau du zoom, où elle occupe entre 28 % et 69 % du cadre selon le barreau retenu, et redevient l'information qu'elle est : la marge opératoire.
...
| `SELECTED WINDOW` | Le zoom, à l'échelle que le créneau retenu impose (§2) et que la note du bandeau nomme : l'intervalle ouvert et l'optimum. **Pas de courbe de coût** — Lemur n'a pas de primitive de ligne et un maillage sur mesure serait hors de proportion ici ; les trois nombres de la ligne de lecture portent la même information. |
...
**Le budget vertical passe, mais de justesse, et l'estimation initiale était fausse.** Elle annonçait ≈ 340 px sur 468 et « la seule page du wizard qui aura de la marge ». Mesuré après coup sur les métriques des fontes embarquées — interligne 15 pour `ibmplexmono-11`, 14 pour le 10, l'inset bas de `newInputField`, et les 192 px de la pile du widget —, la page totalise **459 px sur 468**. Elle tient, avec **9 px de marge**.

C'est peu, et rien à l'écran ne le signale : ce wizard ne clippe pas, donc la prochaine ligne ajoutée ici atterrira sur le footer sans la moindre erreur. Quiconque veut ajouter quelque chose à cette page doit d'abord retirer autre chose.
...
**Le coût du recalcul a été mesuré depuis, et le raisonnement ci-dessus était juste par évaluation et faux par conclusion.** Les 250 évaluations coûtent **28 ms** au total et un recalcul complet de trois opportunités **4 à 9 ms** en régime établi — pas des microsecondes, mais bien dans le budget d'une frame. Ce que la conclusion « donc sur le thread de rendu » n'avait pas mesuré, c'est le **premier accès à ITRF** : `FramesFactory.getITRF` charge à cet instant les données d'orientation terrestre, chronométré à **8 199 ms** sur cette machine, suivi d'un second palier d'environ **550 ms** de JIT et de caches de premier solve. Ce coût n'est pas une propriété du critère — il est payé une fois par JVM, sur le thread qui touche ITRF le premier, et il n'est pas né avec la timeline : avant elle le premier toucher était `EarthMission` à la création de mission, déjà sur le thread de rendu. La sous-page n'a fait que l'avancer et le rendre visible.

**Il est désormais préchauffé au démarrage**, hors thread de rendu : `OrekitService.warmUpFrames()` pour la frame, `EarthLaunchWindowPlanner.warmUp()` pour le solve jetable, lancés par `OrbitLabApplication` sur un thread démon nommé. Mesuré sur le même chemin que la sous-page emprunte : premier solve utilisateur à **9 134 ms sans préchauffage contre 40 ms avec**, puis 16 et 9 ms. Le choix du polling tient donc, mais pour une raison qui est maintenant mesurée au lieu d'être supposée.
...
D'où la séparation. Les validateurs marquent, sans rien monter. Un `RefusedPage.choose(date, durée, inclinaison, nœud)` — fonction pure, testable sans `AssetManager`, à côté de `RaanEntry` et sur le précédent de `MissionDisplayPanelRules` — dit quelle page révéler. Et un `revealRefusal()` la monte, en lisant les marques laissées plutôt qu'en relançant les contrôles. `MissionWizardWidget` l'appelle **deux fois**, et les deux servent : une fois en fin de `parametersRefused()`, qui couvre le refus levé depuis l'étape des paramètres elle-même ; une fois après le `showStep`, qui est la seule à survivre au retour à la page principale. Il est idempotent et ne relance rien, donc l'appeler deux fois sur le même chemin ne coûte rien.
