...
**F. Le refus s'ouvre lui-même.** `validateTargetPlane()` couvre aujourd'hui l'inclinaison et le nœud ensemble ; il se scinde, la vérification du nœud suivant son champ. `StepParameters.validateTargetPlane()` lance les deux et, quand celle du nœud refuse, bascule lui-même sur la page planning avant de rendre la raison. `MissionWizardWidget.parametersRefused()` n'apprend rien : l'étape possède ses pages, donc elle possède la décision de laquelle un refus révèle. La règle vaut aussi pour l'étape `LAUNCHER`, qui re-teste les paramètres parce que le stepper permet de les survoler.
