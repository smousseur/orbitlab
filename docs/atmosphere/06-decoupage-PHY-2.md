...
| ~~**L2**~~ | Isp recalibrée (`DT-13`) — mesure livrée ; **implémentation (Isp + re-baseline) livrée à `L3`** ([`09`](09-conception-L2-PHY-2.md) §6) | oui — attribuable à l'Isp | dette explicitée ; le catalogue passe à 298 en `L3` |
| ~~**L3**~~ | Autorité d'ascension (`DT-21`+`DT-20`) = fix `BUG-25` — **livré le 2026-09-11** (mécanisme seul, `W = 0,5` inchangé ; B-check vert ; `DT-15` sans escalade ; [`10`](10-conception-L3-PHY-2.md)) | oui — attribuable à l'arbitrage | `testFalconHeavyOptimizedTransfer` ré-activé et vert + 3 gates re-baselinés (mode record) + B-check drag-on |
